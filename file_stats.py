...
WORD_TABLE_HEADERS = [
    'Урок', 'Слово', 'Эфф.', 'Попыток', 'Верно', 'Ошибок', 'Hard', 'Due',
]
...
def _configure_report_table(table):
    """Только просмотр: правки в ячейках не сохраняются в БД."""
...
class StatsWindow(QWidget):
    def __init__(self):
        ...
        header = QLabel(
            f'Кандзи в Dictio (уникальных): {kanji_count}   |   '
            f'Записей в Kana: {kana_count}'
        )
    ...
    def _source_combo(self, with_all=False):
        combo = QComboBox()
        if with_all:
            combo.addItem('Все таблицы', None)
    ...
    def _build_slices_tab(self):
        ...
        self.slice_type.addItem('По урокам', 'lesson')
        self.slice_type.addItem('По части речи', 'sush')
        self.slice_type.addItem('По направлению', 'direction')
        self.slice_type.setStyleSheet(styles.combobox)
        bar.addWidget(QLabel('Таблица:'))
        bar.addWidget(self.slice_source)
        bar.addWidget(QLabel('Разрез:'))
    ...
    def _build_words_tab(self):
        ...
        self.words_sort.addItem('Самые сложные', True)
        self.words_sort.addItem('Самые лёгкие', False)
        ...
        bar.addWidget(QLabel('Мин. попыток:'))
    ...
    def _build_srs_tab(self):
        ...
        self.tabs.addTab(w, 'Очередь SRS')
    ...
    def _build_directions_tab(self):
        ...
        bar.addWidget(QLabel('Мин. попыток на направление:'))
        ...
        self.tabs.addTab(w, 'Направления')
    ...
    def refresh_overview(self):
        ...
        for d in data:
            rows.append({
                ...
                'Hard': d['hard'],
                'Сироты': d['orphaned'],
                'Проверено %': f"{d['pct_tested']:.1f}%",
                'Ср. эфф.': format_pct(d['avg_efficiency']),
                '_Всего_sort': d['total'],
                '_Due сегодня_sort': d['due'],
            })
        _fill_table(
            self.overview_table,
            ['Таблица', 'Всего', 'Проверено', 'Не тестиров.', 'Due сегодня', 'Hard', 'Сироты', 'Проверено %', 'Ср. эфф.'],
            rows,
        )
        ...
        try:
            if slice_type == 'lesson':
                ...
                rows = [{
                    'Урок': x['lesson'],
                    'Слов': x['total'],
                    'Проверено %': f"{x['tested_pct']:.1f}%",
                    'Due': x['due'],
                    'Hard': x['hard'],
                    'Ср. эфф.': format_pct(x['avg_efficiency']),
                    'Дней с посл.': x['days_since'] if x['days_since'] is not None else '—',
                    'Попыток': x['attempts'],
                    '_Слов_sort': x['total'],
                    '_Дней с посл._sort': x['days_since'] if x['days_since'] is not None else 99999,
                } for x in data]
                headers = ['Урок', 'Слов', 'Проверено %', 'Due', 'Hard', 'Ср. эфф.', 'Дней с посл.', 'Попыток']
                ...
                headers = ['Часть речи', 'Слов', 'Проверено %', 'Due', 'Hard', 'Ср. эфф.', 'Попыток']
            else:
                ...
                rows = [{
                    ...
                    'Ср. эфф.': format_pct(x['avg_efficiency']),
                    'Попыток': x['attempts'],
                    '_Записей_sort': x['total'],
                    '_Ср. эфф._sort': x['avg_efficiency'] if x['avg_efficiency'] is not None else -1,
                } for x in data]
                headers = ['Направление', 'Записей', 'Due', 'Hard', 'Ср. эфф.', 'Попыток']
        finally:
            conn.close()
    ...
    def _word_rows(self, items):
        ...
        for x in items:
            lesson = x.get('lesson')
            rows.append({
                'Урок': lesson if lesson is not None else '—',
                'Слово': x['label'],
                'Эфф.': format_pct(x['efficiency']),
                'Попыток': x['attempts'],
                'Верно': x['total_right'],
                'Ошибок': x['total_wrong'],
                'Hard': 'да' if x['hard'] else '',
                'Due': 'да' if x['due'] else '',
                '_Эфф._sort': x['efficiency'] if x['efficiency'] is not None else -1,
                '_Попыток_sort': x['attempts'],
            })
        return rows
    ...
    def refresh_words(self):
        ...
        _fill_table(self.words_table, WORD_TABLE_HEADERS, rows)
    ...
    def refresh_srs(self):
        ...
        if row:
            self.srs_summary.setText(
                f'В очереди на сегодня: {total_due} слов (из {row["total"]}, '
                f'не тестировалось: {row["never"]}, hard: {row["hard"]})'
            )
        else:
            self.srs_summary.setText(f'В очереди на сегодня: {total_due} слов')
        rows = self._word_rows(data)
    ...
    def refresh_stuck(self):
        ...
        for x in data:
            rows.append({
                'Урок': x.get('lesson') if x.get('lesson') is not None else '—',
                'Слово': x['label'],
                'Причина': x['reasons'],
                'Эфф.': format_pct(x['efficiency']),
                'Ошибок всего': x['total_wrong'],
                'В сессии wrong': int(x['directions'] and max(
                    (int(s.get('wrong', 0) or 0) for s in x['directions'].values()), default=0
                )),
                '_Ошибок всего_sort': x['total_wrong'],
            })
        _fill_table(
            self.stuck_table,
            ['Урок', 'Слово', 'Причина', 'Эфф.', 'Ошибок всего', 'В сессии wrong'],
            rows,
        )
    ...
    def refresh_asymmetry(self):
        ...
        for x in data:
            rows.append({
                'Урок': x.get('lesson') if x.get('lesson') is not None else '—',
                'Слово': x['label'],
                'Разброс': format_pct(x['spread']),
                'Слабее': f"{x['worst_dir']} ({format_pct(x['worst_eff'])})",
                'Лучше': f"{x['best_dir']} ({format_pct(x['best_eff'])})",
                'Все направления': x['dir_detail'],
                '_Разброс_sort': x['spread'],
            })
        _fill_table(
            self.asym_table,
            ['Урок', 'Слово', 'Разброс', 'Слабее', 'Лучше', 'Все направления'],
            rows,
        )
