...
class TabFraze(QTabWidget):
    ...
    def __init__(self, df_w, xl,parent=None):
        ...
        self.df_fr = df_w
        self.df_fr['Num'] = self.df_fr['Num'].astype(int)

        for i in range(len(self.df_fr)):
            if self.df_fr['Num'].duplicated()[i]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Critical)
                msg.setText(f"Num {self.df_fr['Num'].iloc[i]} is duplicated in the table 'Dictio'")
        ...
        self.populate_table(self.df_fr)
    ...
    def update_data(self, item):
        ...
        self.df_fr['Num'] = pd.to_numeric(self.df_fr['Num'], errors='coerce')
        matching_rows = self.df_fr[self.df_fr['Num'] == num]
        if matching_rows.empty:  # if no such row exists
            return  # exit the function
        index_in_df_w = matching_rows.index[0]
        self.df_fr.loc[index_in_df_w, self.df_fr.columns[col]] = new_value
    ...
    def add_data(self):
        ...
        if lesson =='':
            ...
            if self.df_fr['Num'].empty:
                new_num = 1
            else:
                self.df_fr['Num'] = self.df_fr['Num'].astype(int)
                last_num = int(self.df_fr['Num'].max())
                new_num = last_num + 1
            new_data = DataFrame([[lesson, new_num, kanji, read, trans]], columns=['Lesson', 'Num', 'Kanji', 'Read', 'Trans'])
            self.df_fr = concat([self.df_fr, new_data], ignore_index=True)
            self.sheets['Frazes'] = self.df_fr
    ...
    def delete_data(self):
        ...
        if current_row != -1: # if a row is selected
            ...
            for i in range(len(self.df_fr)):
                if int(self.df_fr['Num'].iloc[i]) == num:
                    self.current_row_for_delete = self.df_fr.iloc[i] # save the row that will be deleted
                    self.df_fr = self.df_fr.drop([i])
                    break
            self.df_fr = self.df_fr.reset_index(drop=True) 
            self.sheets['Frazes'] = self.df_fr 
            self.table.removeRow(current_row) # Remove the row from the table

    def undo_data(self):
        if self.current_row_for_delete is not None:
            self.df_fr = concat([self.df_fr, self.current_row_for_delete], ignore_index=True)
            self.sheets['Frazes'] = self.df_fr
            self.populate_table(self.df_fr)

    def save_data(self):
        non_numeric_lessons = self.df_fr[~self.df_fr['Lesson'].astype(str).str.isnumeric()]['Lesson'].unique()
        if non_numeric_lessons:
            ...
            self.sheets['Frazes'] = self.df_fr.copy()
            with ExcelWriter('J_e_all_my.xlsx', mode='w') as writer:
                for sheet_name, sheet_df in self.sheets.items():
                    sheet_df.to_excel(writer, sheet_name=sheet_name, index=False)
