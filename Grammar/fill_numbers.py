...
def ten_man():
    ...
    # Handling thousands (канзи: 三千/八千, хирагана: さんぜん/はっせん)
...
def multi_million():
    ...
    rand_numb = random.randint(100, 1000)  # 100–1000 万
    man = rand_numb
    ...
    for i, digit in enumerate(str(man)[::-1]):
        ...
        if i == 0:
            ...
            kanji = numb_kanji[digit] + numb_kanji[10] + kanji
            hiragana = numb_hiragana[digit] + numb_hiragana[10] + hiragana
        elif i == 2:
            kanji = numb_kanji[digit] + numb_kanji[100] + kanji
            if digit in special_hiragana:
                hiragana = special_hiragana[digit] + hiragana
            else:
                hiragana = (numb_hiragana[digit] if digit != 1 else '') + numb_hiragana[100] + hiragana

    kanji += numb_kanji[10000]
    hiragana += numb_hiragana[10000]
...
def hundred_million():
    numb_kanji = {0: '', 1: '一', 2: '二', 3: '三', 4: '四', 5: '五', 6: '六', 7: '七', 8: '八', 9: '九', 10: '十', 100: '百', 1000: '千', 10000: '万'}
    numb_hiragana = {0: '', 1: 'いち', 2: 'に', 3: 'さん', 4: 'よん', 5: 'ご', 6: 'ろく', 7: 'しち', 8: 'はち', 9: 'きゅう', 10: 'じゅう', 100: 'ひゃく', 1000: 'せん', 10000: 'まん'}
    special_hiragana = {3: 'さんびゃく', 6: 'ろっぴゃく', 8: 'はっぴゃく', 3000: 'さんぜん', 8000: 'はっせん'}

    rand_numb = random.randint(1000, 10000)  # 1000–10000 → 10,000,000–100,000,000 (in man: 1000–10000 万)
    man = rand_numb

    kanji = ''
    hiragana = ''

    # Собираем число 万 (напр. 8254) справа налево: разряды единицы, десятки, сотни, тысячи
    for i, digit in enumerate(str(man)[::-1]):
        digit = int(digit)
        if digit == 0:
            continue
        if i == 0:
            ...
            # сотни: 二百 (спец. чтения 三百→さんびゃく, 六百→ろっぴゃく, 八百→はっぴゃく)
            kanji = numb_kanji[digit] + numb_kanji[100] + kanji
            if digit in (3, 6, 8):
                hiragana = special_hiragana[digit] + hiragana
            else:
            ...
            # тысячи: 八千 (спец. 三千→さんぜん, 八千→はっせん)
            kanji = numb_kanji[digit] + numb_kanji[1000] + kanji
            if digit * 1000 in special_hiragana:
                hiragana = special_hiragana[digit * 1000] + hiragana
            else:
                hiragana = (numb_hiragana[digit] if digit != 1 else '') + numb_hiragana[1000] + hiragana
